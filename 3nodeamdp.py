        ...
        for u, action in enumerate(actions):
            ...
            if b == 't':
                ...
                for k in range(n):
                    ...
                    if action != 0:
                        P_iu[len(bs)-1] += 1/n
                        R_i[u] += get_reward(vehicle_state, intruder_state, action) / n
                    else:
                        # sample z ~ p(x|x')
...
def main(save=False):
    ...
    V, P, R, W = AMDP(bs, T, B, tp, fp)

    if save:
        folder = '/home/matt/Dropbox/Research/SavedResults/3nodeamdp/'
        np.save(folder + 'V', V)
        np.save(folder + 'P', P)
        np.save(folder + 'R', R)
        np.save(folder + 'W', W)
