using POMDPs, Distributions, POMDPToolbox
...
function rasmussen_rewards(mdp::GridWorld,reward_states::Vector{GridWorldState})
    rasmussen_rs = Vector{GridWorldState}(0)
    for x = 1:mdp.size_x
        for y = 1:mdp.size_y
            for s in reward_states
                if s.x == x && s.y == y
                    push!(rasmussen_rs,GridWorldState(x,y,true)
                    ####### GET REWARDS WORKING, NEED NON-TERMINAL REWARDS TO BE -.04... OR WHATEVER IT IS IN THE BOOK

            push!(rasmussen_rs,GridWorldState(x,y,false)
    for s in reward_states

rasmussen_rs = [GridWorldState(1,1), GridWorldState(1,2), GridWorld
rasmussed_gw =
