...
p1 = Plots.plot(trusted_gp,β=beta_val,linecolor=:blue,fillcolor=:blue,lw=3,obsv=show_obs,fillalpha=0.2,label="trusted")
Plots.plot!(candidate_gp,β=beta_val,linecolor=:red,fillcolor=:red,lw=2,obsv=show_obs,fillalpha=0.2,label="candidate")
...
for i = 1:length(points_of_interest_x)
    point_label = '@' + i
    Plots.annotate!(points_of_interest_x[i],points_of_interest_y[i],text("$point_label",:black,:right,:bottom))
end
...
Plots.annotate!(0.1,0.65,text("* Calculate mean difference weighted by std",10,:center,:left))
Plots.annotate!(0.5,0.5,text(L"wd=\frac{m(c)/s(c)-m(t)/s(t)}{range(rwd)}",10,:center,:center))
Plots.annotate!(0.1,0.3,text("* SQ is a scaled Sigmoid on wd",10,:center,:left))
Plots.annotate!(0.5,0.15,text(L"SQ=\frac{2exp(wd)}{(exp(wd)+1)}",10,:center,:center))
#  annotate!(0.,0.,text("empty",:black,:center))
pe3 = Plots.plot(framestyle = :none)
Plots.annotate!(0.1,0.65,text("* In these plots:",10,:center,:left))
Plots.annotate!(0.5,0.5,text(LaTeXString(@sprintf("range(rwd) = %s",solver_reward_range)),10,:center,:center))
#  annotate!(0.,0.,text("empty",:black,:center))

p1notes = Plots.plot(framestyle = :none)
Plots.annotate!(0.5,0.5,text(@sprintf("SQ: %0.3f\n%0.3f\n%0.3f\n%0.3f",SQ[1][1],SQ[1][2],SQ[1][3],SQ[1][4]),10,:center,:center))

p2notes = Plots.plot(framestyle = :none)
Plots.annotate!(0.5,0.5,text("SQ: $(@sprintf("%0.3f",SQ[2][1]))",10,:center,:center))

p3notes = Plots.plot(framestyle = :none)
Plots.annotate!(0.5,0.5,text("SQ: $(@sprintf("%0.3f",SQ[3][1]))",10,:center,:center))

p4notes = Plots.plot(framestyle = :none)
Plots.annotate!(0.5,0.5,text("SQ: $(@sprintf("%0.3f",SQ[4][1]))",10,:center,:center))
